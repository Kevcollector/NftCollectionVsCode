...
def collection(author, collection_name, heading, *excelsheetname):
    ...
    def normalServic(authors, all, resales, FirstSale, Holders, holders, *excelsheetname):
                ...
                if assitID1 != assitID2:
                    ...
                    for i in s:
                        if "bored" in word:
                            if i == "#1":
                                shark1.append(
                                    (checker, nft_name, number_of_nft))
                            elif i == "#2":
                                shark2.append(
                                    (checker, nft_name, number_of_nft))
                            elif i == "#3":
                    ...
                    if "bored " in word:
                        ...
                        ws3.cell(row=rowx, column=5 + count).value = nft_name + \
                            '  (#' + number_of_nft + ')   '
                        count = count + 1
        ...
        print(shark3)
        print(shark4)
        shark1s = pd.DataFrame(data=shark1, columns=[
                               "account", "Nft name", "Nft number"])
        shark2s = pd.DataFrame(data=shark2, columns=[
                               "account", "Nft name", "Nft number"])
        shark3s = pd.DataFrame(data=shark3, columns=[
                               "account", "Nft name", "Nft number"])
        shark4s = pd.DataFrame(data=shark4, columns=[
                               "account", "Nft name", "Nft number"])
