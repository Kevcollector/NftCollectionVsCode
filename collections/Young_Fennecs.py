    ...
    def normalServic(authers, all, resales, FirstSale, Holders, holders,*excelsheetname):
        ...
        while len(authers_['data']) !=0:
            ...
            for data_info in authers_['data']:
                ...
                authers_list.append([seller, fixed,Royal, buyer, number_of_nft, name, local_time])
            time.sleep(0.4)
            authers = ("https://proton.api.atomicassets.io/atomicmarket/v1/sales?state=3&account={}"
            ...
            authers_ = json.loads(authers)
        auctions = (
            "https://proton.api.atomicassets.io/atomicmarket/v1/auctions?state=3&seller={}&collection_name={}&page=1&limit=100&order=desc&sort=created".format(auther,
                collection_name))
        ...
        for data_info in acutions_['data']:
            ...
            auctionss_list.append([seller, fixed,Royal, buyer, number_of_nft, name, local_time])
        ...
        resells = ("https://proton.api.atomicassets.io/atomicmarket/v1/sales?state=1%2C3&seller_blacklist"
                   "={}&buyer_blacklist={}&collection_name={}&page=1&limit=100&order"
                   "=desc&sort=updated".format(auther, auther, collection_name))
        auctions=("https://proton.api.atomicassets.io/atomicmarket/v1/auctions?state=3&seller_blacklist={}&collection_name={}&page=1&limit=100&order=desc&sort=created".format(auther,collection_name))
        acutionss=requests.get(auctions).text
        acutions_=json.loads(acutionss)
        auctions_list=[]
        all = requests.get(resells).text
        ...
        while len(all_['data']) != 0:
            for data_info in all_['data']:
                ...
                all_list.append([seller, buyer, fixed,Royal, name, number_of_nft, local_time])
            time.sleep(.6)
            ...
            for data_info in acutions_['data']:
                ...
                local_time = datetime.utcfromtimestamp(timefixe).strftime('%d-%m-%Y %H:%M:%S')
                auctions_list.append([seller, buyer, fixed,Royal, name, number_of_nft, local_time])
        ...
        for r in dataframe_to_rows(names_df, index=False):
            count = count + 1
        for i in range (0,count-1):
            total=(names_df.loc[names_df.index[i],"price paid usd"]*names_df.loc[names_df.index[i], "Fee's"])+total
...
collection(auther,collection_name1,heading,excelsheetname1)
