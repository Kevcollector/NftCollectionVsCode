...
def collection(author, collection_name, heading, *excelsheetname):
    ...
    if collection_name == "255111242253":
        authors = (
            "https://proton.api.atomicassets.io/atomicmarket/v1/buyoffers?state=3&account={}"
            "&collection_name={}&page=1&limit=100&order=desc&sort=updated".format(
                author, collection_name
            )
        )
    ...
    def normalServic(
        ...
        os.chdir(path)
        authors = "https://proton.api.atomicassets.io/atomicmarket/v1/sales?state=3&account={}&collection_name={}&page=1&limit=100&order=desc&sort=created".format(
            author, collection_name
        )
        authors = requests.get(authors).text
        authors_ = json.loads(authors)
        ...
        if collection_name == "255111242253":
            authors = "https://proton.api.atomicassets.io/atomicmarket/v1/buyoffers?state=3&seller={}&collection_name={}&page=1&limit=100&order=desc&sort=created".format(
                author, collection_name
            )
            print(authors)
            authors = requests.get(authors).text
        ...
        for i in range(0, count - 1):
            total = (
                name_df.loc[name_df.index[i], "price listed usd"]
                * name_df.loc[name_df.index[i], "Fee's"]
            ) + total
        dims = {}
        for row in ws.rows:
            for cell in row:
        ...
        for i in range(0, count - 1):
            total = (
                auctions_df.loc[auctions_df.index[i], "price listed usd"]
                * auctions_df.loc[auctions_df.index[i], "Fee's"]
            ) + total
        for row in ws.rows:
            for cell in row:
                if cell.value:
        ...
        auctions = "https://proton.api.atomicassets.io/atomicmarket/v1/auctions?state=3&seller_blacklist={}&collection_name={}&page=1&limit=100&order=desc&sort=created".format(
            author, collection_name
        )
        acutionss = requests.get(auctions).text
        acutions_ = json.loads(acutionss)
        ...
        for i in range(0, count - 1):
            total = (
                names_df.loc[names_df.index[i], "price paid usd"]
                * names_df.loc[names_df.index[i], "Fee's"]
            ) + total
...
author = "goofyllamas"
universe = "Young Fennecs"
collection2 = "Goofy Llama"

heading = "{} Collection".format(universe)
collection_name2 = "255111242253"


excelsheetname1 = "{}.xlsx".format(collection2)

collection(author, collection_name2, heading, excelsheetname1)
