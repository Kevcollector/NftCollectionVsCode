...
def collection(author, collection_name, heading, select, *excelsheetname):
    ...
    def normalServic(authors, all, resales, FirstSale, Holders, holders, select, *excelsheetname):
        global Royal
        os.chdir(path)
        authors = ("https://proton.api.atomicassets.io/atomicmarket/v1/sales?state=3&account={}"
                   "&collection_name={}&page=1&limit=100&order=desc&sort=updated".format(author, collection_name))
        ...
        while len(all_['data']) != 0:
            for data_info in all_['data']:
                ...
                all_list.append([seller, buyer, fixed, name,
                                number_of_nft, local_time])
            time.sleep(.6)
    ...
    normalServic(authors, all, resales, FirstSale,
                 Holders, holders, select, *excelsheetname)
