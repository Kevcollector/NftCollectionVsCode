...
print(
    "\t\tIn the intrest of speeding up the code you please select from the following list\n\t\tenter the number for the collection you want\n"
)
choice = input(
    "1.\tDino\n2.\tBaby Dino\n3.\tBaby Dino Fussion\n4.\tSerpents\n5.\tKrok\n9. for all :"
)
...
def collection(author, collection_name, heading, select, *excelsheetname):
    global Royal
    ...
    parents = requests.get(
        ...
        "=desc&sort=asset_id".format(collection_name)
    ).text
    listed = "https://proton.api.atomicassets.io/atomicmarket/v1/sales?state=1&collection_name={}&page=1&limit=100&order=asc&sort=price".format(
        collection_name
    )
    ...
    def normalServic(
        authors, all, resales, FirstSale, Holders, holders, select, *excelsheetname
    ):
        global Royal
        os.chdir(path)
        authors = (
        ...
        if select == "2":
            for data_info in holders_["data"]:
                ...
                people = requests.get(people).text
                ...
                for data_info in people_["data"]:
                    ...
                    supply = data_info["template"]["max_supply"]
                    word = word.lower()
                    if assitID1 != assitID2:
                        ...
                        if collection_name == "355422121445":
                            if "rare" or "gold" in word:
                                points = points + 5
                            if "uncommon" in word:
                                points = points + 4
                            if "common" in word:
                        ...
                        if collection_name == "423534451325":

                            if supply == "1":
                                if done == "Gold" or done == "Silver":
                                    points += 5
                                else:
                                    points += 4
                            elif supply == "3":
                ...
                ws3.cell(row=rowz, column=3).value = points

                ws3.cell(row=1, column=3).value = "points"

                holder_list.append([checker, holders_amount, points])
        ...
        listed = "https://proton.api.atomicassets.io/atomicmarket/v1/sales?state=1&collection_name={}&page=1&limit=100&order=asc&sort=updated".format(
            collection_name
        )
        listed = requests.get(listed)
        listed_ = json.loads(listed.text)
        ...
        while len(listed_["data"]) != 0:
            ...
            time.sleep(0.4)
            listed = "https://proton.api.atomicassets.io/atomicmarket/v1/sales?state=1&collection_name={}&after={}&page=1&limit=100&order=asc&sort=updated".format(
        ...
        for r in dataframe_to_rows(listed_df, index=False):
            ...
            if count > 2:
                ...
                wsL.cell(
                    row=count - 1, column=6
                ).value = '=HYPERLINK("{}", "{}")'.format(link, link)
                wsL.cell(row=count - 1, column=6).style = "Hyperlink"
        count = count + 1
        link = wsL.cell(row=count - 1, column=6).value
        wsL.cell(row=count - 1, column=6).value = '=HYPERLINK("{}", "{}")'.format(
            link, link
        )
        wsL.cell(row=count - 1, column=6).style = "Hyperlink"
        ...
        if collection_name == "133523522522":
            ...
            parents = requests.get(
                "https://proton.api.atomicassets.io/atomicassets/v1/assets?collection_name={}&page=1&limit=100&order"
                "=desc&sort=updated".format(collection_name)
            ).text
            parents_ = json.loads(parents)
            while len(parents_["data"]) != 0:
                ...
                parents = requests.get(
                    ...
                    "=desc&sort=updated".format(collection_name, timef)
                ).text
                parents_ = json.loads(parents)
            s_df = pd.DataFrame(
                data=parents_list, columns=["Name ", "first parent", "second parent"]
            )
...
if choice == "1" or choice == "9":
    collection(author, collection_name, heading, select, excelsheetname5)
if choice == "2" or choice == "9":
    collection_name = collection_name2
    if choice == "9":
        time.sleep(4)
    collection(author, collection_name, heading, select, excelsheetname2)
if choice == "3" or choice == "9":
    collection_name = collection_name3
    if choice == "9":
        time.sleep(4)
    collection(author, collection_name, heading, select, excelsheetname3)
if choice == "4" or choice == "9":
    collection_name = collection_name5
    if choice == "9":
        time.sleep(4)
    collection(author, collection_name, heading, select, excelsheetname6)
if choice == "5" or choice == "9":
    collection_name = "355422121445"
    if choice == "9":
        time.sleep(4)
    collection(author, collection_name, heading, select, excelsheetname7)
