    ...
    def normalServic(authors, all, resales, FirstSale, Holders, holders, *excelsheetname):
        ...
        while len(authors_['data']) != 0:
            ...
            for data_info in authors_['data']:
                ...
                authors_list.append(
                    [seller, fixedC, fixedX, buyer, number_of_nft, name, local_time])
            time.sleep(0.4)
            authors = ("https://proton.api.atomicassets.io/atomicmarket/v1/sales?state=3&account={}"
                                    ...
                                    if temp1 == "0":
                                        ...
                                        try:
                                            ...
                                            ws3.cell(row=rowz, column=3).value = int(
                                                prices) + int(0)
                                try:
                                    ws3.cell(row=rowz, column=4).value = int(
                                        lookup[nft_name])+int(temp2)
                                except:
                                    try:
                                        ws3.cell(
                                            row=rowz, column=4).value = 0 + int(temp2)
                                    except:
                                        ws3.cell(
                                            row=rowz, column=4).value = 0 + int(0)
